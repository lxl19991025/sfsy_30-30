...
if os.path.isfile('notify.py'):
    ...
    print("加载通知服务失败!")
desp_msg = ""
desp_msg = desp_msg + "本期可换顺丰23免单卷账号如下：" +'\n'
print(desp_msg)
send_msg = ''
one_msg = ''
personal_phone = 18740210480
end_time = 21212
...
class RUN:
    ...
    def login(self, sfurl):
        ...
        if self.phone != '':
            print("--------------------------")
            global personal_phone
            personal_phone = self.mobile
            print(self.mobile)
            print(personal_phone)
            print("--------------------------")
    ...
    def honey_indexData(self, END=False):
        ...
        if response.get('success') == True:
            ...
            print(usableHoney)
            if usableHoney >= 4600:
                desp_msg = desp_msg + "账号：" + str(personal_phone) + '\n'
            print("+++++++++++++++++++++++++++++++++")
...
def pushplus(_item, _message):
    ...
    _d = {
        "token": '4d1126005be44209b2cb4addef3fba45',
        "title": _item,
        "content": _message,
        "topic": 'sfsy'
    }
...
✨✨✨ @Author CHERWIN✨✨✨
    ...
    if len(tokens) > 0:
        ...
        print(send_msg)
        desp_msg = desp_msg + "本期活动结束时间：" + str(end_time)  +'\n'
        desp_msg = desp_msg + "不够23免单的可以换其他数额！！！！！！" +'\n'
        desp_msg = desp_msg + "请在本期活动结束前完成兑换！！！！！！" +'\n'
        desp_msg = desp_msg + "账号失效联系QQ：1831921455" +'\n'
        print(desp_msg)
        pushplus("顺丰签到信息",send_msg)
        pushplus("顺丰23免单卷兑换提醒！！！",desp_msg)
